                    ...
                    <form method="POST" action="{{ route('tasks.update',$task) }}">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                                          :value="old('title',$task->title)"  autofocus autocomplete="title"/>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="description" class="block mt-1 w-full" type="text" name="description"
                                          :value="old('description',$task->description)"  autofocus autocomplete="description"/>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="deadline_at" class="block mt-1 w-full" type="date" name="deadline_at"
                                          :value="old('deadline_at',$task->deadline_at)"  autocomplete="deadline"/>
                            ...
                            <select id="client_id" name="client_id" class="block mt-1 w-full">
                                @foreach($clients as $client)
                                    <option value="{{$client->id}}"
                                        @selected(old('client_id', $task->client_id) == $client->id)>
                                        {{$client->company_name}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="project_id" name="project_id" class="block mt-1 w-full">
                                @foreach($projects as $project)
                                    <option value="{{$project->id}}"
                                        @selected(old('title', $task->title) == $project->id)>
                                        {{$project->title}}
                                    </option>
                                @endforeach
                            </select>
